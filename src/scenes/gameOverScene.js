...
export default class GameOverScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.saveText = this.add.text(0, 0, 'Save Score', {
      fontSize: '24px',
      fill: '#fff',
    });
    ...
    this.saveButton.on('pointerdown', () => {
      if (!this.saved) {
        saveScore(input.value, this.score).then((resp) => {
          this.updateText = this.add.text(400, 100, `${resp.result}`, {
            fontSize: '32px',
            fill: '#fff',
          });
        ...
        this.updateText = this.add.text(400, 100, 'Score Alredy Saved', {
          fontSize: '32px',
          fill: '#fff',
        });
        this.updateText.setOrigin(0.5, 0.5);
    ...
    this.leaderboardText = this.add.text(0, 0, 'Leaderboard', {
      fontSize: '24px',
      fill: '#fff',
    });
